...
#[juniper::object]
impl QueryRoot {
    fn orderHistory(account_id: String, sap_contact_id: String) -> FieldResult<Root> {
        let mut rt = tokio::runtime::Runtime::new().unwrap();
        let future = stuff(account_id, sap_contact_id);
...
async fn stuff(account_id: String, sap_contact_id: String) -> Result<Root, SolrError> {
    ...
    let mut owned_sap_contact_id_key_value = "sapContactId:".to_owned();
    owned_sap_contact_id_key_value.push_str(&*sap_contact_id.borrow());

    let solr_request = solr_client
        .select("orders")?
        .rows(10)
        .q(&*owned_account_number_key_value)
        .fq(&*owned_sap_contact_id_key_value)
        .fl("*")
        .wt("json")
